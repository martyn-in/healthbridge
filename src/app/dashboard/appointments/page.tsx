        ...
        <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700 shadow-card p-6 text-white flex items-center justify-between flex-wrap gap-4">
          <div>
            <span className="chip chip-teal mb-2 inline-flex items-center gap-1">
              <CalendarIcon className="h-3 w-3" /> Physician Clinical Schedule
            </span>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 active:scale-95 text-white font-bold text-xs shadow-sm transition-all shrink-0"
          >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {appointments.map((apt) => (
              <div
                key={apt.id}
                className="rounded-xl p-5 shadow-card border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 space-y-4 card-hover"
              >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-800 border border-slate-700 shadow-card p-6 text-white flex items-center justify-between flex-wrap gap-4">
        <div>
          <span className="chip chip-teal mb-2 inline-flex items-center gap-1">
            <CalendarIcon className="h-3 w-3" /> Care Coordination
          </span>
          <h1 className="text-2xl font-extrabold tracking-tight">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {appointments.map((apt) => (
            <div
              key={apt.id}
              className={`rounded-xl p-5 shadow-card border transition-all space-y-4 card-hover ${
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="chip chip-teal text-[10px]">{apt.specialty}</span>
                ...
                <span
                  className={`chip text-[10px] ${
                    apt.status === 'Upcoming' ? 'chip-green' : 'chip-amber'
                  }`}
                >
                  {apt.status}
                </span>
              ...
              <form onSubmit={handleBookSubmit} className="space-y-3 text-xs">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 outline-none"
                  >
                    {demoDoctors.map((d) => (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 outline-none"
                    />
                    <input
                      type="text"
                      value={time}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 outline-none resize-none"
                  />
                </div>

                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold transition-all active:scale-[0.98]"
                >
                  Confirm Appointment
                </button>
              </form>
